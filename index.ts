...
async function main() {
  try {
    const oldFile: ClassUtils|null =
        update_offsets ? new ClassUtils(old_dump) : null;
    const newFile: ClassUtils|null =
        update_offsets || update_fields ? new ClassUtils(new_dump) : null;
    const offsetInfo: FileOffsets|undefined =
        update_offsets ? await getOffsets(offset_file) : undefined;
    const fieldInfo: FileOffsets|undefined =
        update_fields ? await getOffsets(field_file) : undefined;
    ...
    if (update_offsets && offsetInfo && offsetNames) {
      ...
      await Promise.all(
          OffsetPatterns.map(
              async (pattern, index) => pushOffset(pattern, index, {
                oldFile,
                newFile,
                offsetInfo,
                newContent,
      ...
      console.log(
          chalk.grey(
              `Total offsets updated: ${chalk.green(newOffsets.length)}`),
      );

      console.log(
          chalk.grey(
              `Average offset exec time: ${
                  chalk.blue(averageTime.toFixed(3))}ms`,
              ),
      );
    ...
    if (update_fields && fieldInfo) {
      ...
      await Promise.all(
          FieldPatterns.map(
              async (pattern, index) => pushField(pattern, index, {
                newContent,
