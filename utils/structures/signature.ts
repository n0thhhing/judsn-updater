...
class SignatureUtils implements SignatureUtil {
  ...
  async findPatterns(patterns: RegExp[] | RegExp): Promise<string> {
    ...
    const results: string[] = Array.isArray(patterns) ? patterns.flatMap(getMatches) : getMatches(patterns);
  ...
  public async getSignature(
    ...
    try {
      ...
      const idx =
        offset !== null && typeof offset === 'number' ? offset.toString() : offset?.toString() || '';
  ...
  public async getSigOffset(
    ...
    const regex = new RegExp(
      `${
        previousSignature ? `"Signature": "${previousSignature.replace(
          /[.*+?^${}()|[\]\\]/g,
          '\\$&',
        )}",\n      "TypeSignature": "\\S+"\n    \\},\n    \\{\n      "Address": ` : ''
