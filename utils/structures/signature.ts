...
import type {
  Count, Data, FilePath, MethodSignature, Offset, OffsetPattern,
  ReturnedSignature, Signature, SignatureName, SignatureOutput, SignatureUtil,
  Time,} from '../';
...
class SignatureUtils implements SignatureUtil {
      ...
      console.log(
          chalk.grey(
              `getContent(${this.path}): ${
                  chalk.blue(elapsedTime.toFixed(3))}ms`,
              ),
      );
  ...
  public async getSignature(
    ...
    try {
      ...
      const idx: string = offset !== null && typeof offset === 'number'
                              ? offset.toString()
                              : offset?.toString() || '';
